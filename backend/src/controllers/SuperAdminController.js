...
class SuperAdminController {
    ...
    try {
      ...
      let query = db('tenants as t')
        ...
        .select(
          ...
          db.raw(`CASE 
            WHEN bc.access_token IS NOT NULL AND bc.token_expires_at > datetime('now') THEN 'connected'
            WHEN bc.access_token IS NOT NULL AND bc.token_expires_at <= datetime('now') THEN 'expired'
  ...
  async createSupplier(req, res) {
    try {
      const { name, email, phone, company_name, document, address, plan = 'starter' } = req.body;
      ...
      const existingSlug = await db('tenants').where('slug', slug).first();
      if (existingSlug) {
        return res.status(400).json({
          success: false,
          message: 'Já existe um fornecedor com nome similar'
        });
      }

      // Verificar se o email já existe
      const existingEmail = await db('tenants').where('email', email).first();
      if (existingEmail) {
        return res.status(400).json({
          success: false,
